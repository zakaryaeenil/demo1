...
@Component({
  selector: 'app-dossier-enntraitement',
  templateUrl: './dossier-enntraitement.component.html',
  styleUrls: ['./dossier-enntraitement.component.scss']
})
export class DossierEnntraitementComponent implements OnInit {
  ...
  getDossiersALl(){
    ...
    if(this.roles && this.roles.map(r => r.name == 'ADMIN')){
      this.service.getDossiersEntraitement(this.helper).subscribe(
        data =>{
      ...
      this.service.getLoggedInEmployeeFolders(this.helper , 'Entraitement').subscribe(
        data =>{
          this.dossiers = data
          this.rowData$ = data;
        });
      ...
      this.service.getLoggedInClientFolders(this.helper , 'Entraitement').subscribe(
        data =>{
          this.dossiers = data
          this.rowData$ = data;
        });
  ...
  open(content : any) {
    ...
    this.modalService.open(content, { ariaLabelledBy: 'modal-basic-title' }).result.then((result) => {
      ...
      this.closeResult = `Dismissed ${DossierEnntraitementComponent.getDismissReason(reason)}`;
    });
  }
