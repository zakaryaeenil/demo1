...
@Component({
  ...
  styleUrls: ['./dossier-completed.component.scss']
})
export class DossierCompletedComponent implements OnInit {
  ...
  getDossiersALl(){
    ...
    if(this.roles && this.roles.map(r => r.name == 'ADMIN')){
      this.service.getDossiersCompleted(this.helper).subscribe(
        data =>{
      ...
      this.service.getLoggedInEmployeeFolders(this.helper , 'Terminer').subscribe(
        data =>{
          this.dossiers = data
          this.rowData$ = data;
        });
      ...
      this.service.getLoggedInClientFolders(this.helper, 'Terminer').subscribe(
        data =>{
          this.dossiers = data
          this.rowData$ = data;
        });
  ...
  open(content : any) {
    ...
    this.modalService.open(content, { ariaLabelledBy: 'modal-basic-title' }).result.then((result) => {
      ...
      this.closeResult = `Dismissed ${DossierCompletedComponent.getDismissReason(reason)}`;
    });
  }
