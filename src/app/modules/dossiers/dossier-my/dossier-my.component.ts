...
@Component({
  selector: 'app-dossier-my',
  templateUrl: './dossier-my.component.html',
  styleUrls: ['./dossier-my.component.scss']
})
export class DossierMyComponent implements OnInit {
  ...
  getDossiersALl(){
    ...
    if(this.roles && this.roles.map(r => r.name == 'USER')){
      this.service.getLoggedInClientFolders(this.helper , 'All').subscribe(
        data =>{
          this.dossiers = data
          this.rowData$ = data;
        });
      ...
      this.service.getLoggedInEmployeeFolders(this.helper , 'All').subscribe(
        data =>{
          this.dossiers = data
          this.rowData$ = data;
        });
  ...
  open(content : any) {
    ...
    this.modalService.open(content, { ariaLabelledBy: 'modal-basic-title' }).result.then((result) => {
      ...
      this.closeResult = `Dismissed ${DossierMyComponent.getDismissReason(reason)}`;
    });
  }
