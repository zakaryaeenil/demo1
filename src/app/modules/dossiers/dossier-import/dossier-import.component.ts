...
@Component({
  ...
  styleUrls: ['./dossier-import.component.scss']
})
export class DossierImportComponent implements OnInit {
  ...
  getDossiersALl(){
    ...
    if(this.roles && this.roles.map(r => r.name == 'ADMIN')){
      this.service.getDossiersImport(this.helper).subscribe(
        data =>{
      ...
      this.service.getLoggedInEmployeeFolders(this.helper , 'import').subscribe(
        data =>{
          this.dossiers = data
          this.rowData$ = data;
        });
      ...
      this.service.getLoggedInClientFolders(this.helper , 'import').subscribe(
        data =>{
          this.dossiers = data
          this.rowData$ = data;
        });
  ...
  open(content : any) {
    ...
    this.modalService.open(content, { ariaLabelledBy: 'modal-basic-title' }).result.then((result) => {
      ...
      this.closeResult = `Dismissed ${DossierImportComponent.getDismissReason(reason)}`;
    });
  }
